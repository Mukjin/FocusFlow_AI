...
> GitHub Pages는 정적 호스팅이라 서버 함수를 돌릴 수 없어, 이 데모는
> **규칙 엔진 전용**입니다(모든 날짜에 같은 문구).
> AI 구체화까지 보려면 로컬 실행 또는 아래 "서버 함수로 배포하기"를 따라주세요.
...
> ### API 키는 어디에 두는가
>
> 이 앱은 두 가지 모드로 동작하며, **빌드 시점에** 어느 쪽인지 결정됩니다.
>
> | | 키 위치 | 언제 |
> |---|---|---|
> | **직접 모드** | `VITE_GEMINI_API_KEY` (브라우저) | 로컬 개발 |
> | **프록시 모드** | `GEMINI_API_KEY` (서버 전용) | 배포 |
>
> `VITE_` 접두사가 붙은 값은 **번들에 그대로 들어가 공개됩니다.** 그래서 배포본은
> 서버 함수 [`api/gemini.ts`](./api/gemini.ts) 가 키를 쥐고 대신 호출합니다.
> 브라우저에는 키가 전혀 내려가지 않습니다.
>
> 두 변수를 동시에 설정하면 **빌드가 실패합니다.** 실수로 프록시 배포에
> 클라이언트 키를 섞어 넣는 것을 막기 위한 장치입니다.

## 서버 함수로 배포하기 (AI 포함)

GitHub Pages는 정적 호스팅이라 서버 함수를 돌릴 수 없습니다. Vercel처럼
서버리스를 지원하는 곳에 올려야 배포본에서도 AI가 동작합니다.

```bash
npm i -g vercel        # 이미 설치돼 있으면 생략
vercel login           # 브라우저가 열립니다
vercel link            # 프로젝트 연결 (처음 한 번)

# 서버 전용 키 — VITE_ 접두사를 붙이지 마세요
vercel env add GEMINI_API_KEY production
# 클라이언트가 호출할 프록시 경로
vercel env add VITE_AI_ENDPOINT production      # 값: /api/gemini

vercel --prod
```

`api/gemini.ts` 가 하는 일:

- 키를 서버에만 두고 Gemini를 대신 호출
- **프롬프트 조립도 서버에서** — 남이 이 엔드포인트로 임의의 프롬프트를 밀어넣어
  키를 태워 쓰는 것을 막습니다. 클라이언트는 '과목·단계별 개수'만 보냅니다
- 입력 제한(본문 16KB, 목표 12개, 항목 400개)과 IP 단위 빈도 제한(분당 6회)
- 기본은 같은 도메인에서만 호출 가능. 함수를 따로 배포할 때만 `ALLOWED_ORIGIN` 설정

> 엣지 함수는 인스턴스마다 메모리가 따로라 빈도 제한이 완전하지 않습니다.
> 트래픽이 많아지면 KV/Redis 기반 제한이나 인증을 붙여야 합니다.
...
| Gemini 프록시 서버 함수 (키 은닉·프롬프트 서버 조립·입력 제한·빈도 제한) | `api/gemini.ts` |
...
- **서버는 Gemini 프록시 하나뿐입니다.** 일정 생성·저장 등 나머지 로직은 전부 브라우저에서 돕니다.
